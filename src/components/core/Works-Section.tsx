...
import { ExternalLink, ArrowUpRight, ShieldCheck, Calendar, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    return (
        ...
        >
            <div className="relative z-10 space-y-16">
                ...
                <div
                    className={`grid grid-cols-1 lg:grid-cols-12 gap-8 items-end transition-all duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                        hasIntersected ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                    }`}
                >
                    <div className="lg:col-span-9 space-y-4">
                        <div className="w-20 h-[3px] bg-gradient-to-r from-[#00C9FF] to-[#92FE9D]" />
                ...
                <div className="relative w-full">
                    <div
                        ...
                        className="flex overflow-x-auto snap-x snap-mandatory scrollbar-none space-x-0 rounded-3xl pt-2 px-0.5 scroll-smooth"
                            ...
                            return (
                                <div
                                    ...
                                    className={`w-full flex-shrink-0 snap-start snap-always
                                        flex flex-col lg:flex-row gap-8 lg:gap-12 items-stretch 
                                        bg-white dark:bg-zinc-800/40 backdrop-blur-md 
                                        border border-zinc-200 dark:border-zinc-700/60
                                        rounded-3xl overflow-hidden p-6 md:p-8 lg:p-10 
                                ...
                                >
                                    {/* LEFT: Image Canvas */}
                                    <div className="w-full lg:w-[56%] aspect-[16/10] bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-700/50 rounded-2xl overflow-hidden shadow-[inset_0_1px_4px_rgba(0,0,0,0.005)] flex flex-col justify-between p-3 relative transition-all duration-500 group-hover:border-zinc-300 dark:group-hover:border-zinc-600 shrink-0">

                                        {/* Browser Mockup Bar */}
                                        <div className="flex items-center justify-between w-full border-b border-zinc-100 dark:border-zinc-800 pb-2.5 z-20 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm absolute top-0 inset-x-0 px-4 pt-3">
                                            ...
                                            <span className="text-[8px] font-mono font-medium text-zinc-400 dark:text-zinc-500 tracking-tight">
                                                view_0{currentActiveIndex + 1}.config
                                            </span>
                                        </div>

                                        {/* Image Asset Canvas */}
                                        <div className="absolute inset-0 overflow-hidden rounded-2xl pt-10 pb-1 px-1 flex items-center justify-center bg-white dark:bg-zinc-900">
                                            ...
                                            {project.images && project.images.length > 1 && (
                                                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-center gap-1.5 bg-[#1C1C1C]/90 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-white/10 shadow-lg z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 max-w-[85%] overflow-x-auto scrollbar-none">
                                                    {project.images.map((img, imgIdx) => (
                                                        <button
                                                            key={imgIdx}
                                                            onClick={(e) => {
                                                            ...
                                                            className={`relative h-5 w-8 rounded overflow-hidden border transition-all duration-300 shrink-0 ${
                                                                currentActiveIndex === imgIdx
                                    ...
                                    {/* RIGHT: Data Shelf */}
                                    <div className="flex flex-col flex-1 justify-between py-2 space-y-6">
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between gap-2">
                                                <div className="flex items-center gap-1.5 text-[10px] font-bold text-[#00C9FF] uppercase tracking-wider">
                                                    <ShieldCheck className="w-3.5 h-3.5 text-[#92FE9D]" />
                                                    {project.category}
                                                </div>
                                            ...
                                            <div className="space-y-1">
                                                <h3 className="text-2xl sm:text-3xl font-black tracking-tight text-zinc-900 dark:text-zinc-100 group-hover:text-[#00C9FF] transition-colors duration-300 leading-tight">
                                                    {project.title}
                                                </h3>
                                                <p className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 tracking-tight leading-snug">
                                                    {project.subtitle}
                                                </p>
                                            </div>

                                            <p className="text-[13px] text-zinc-600 dark:text-zinc-400 leading-relaxed font-normal line-clamp-5 lg:line-clamp-6">
                                                {project.description}
                                            </p>
                                        </div>
                                        ...
                                        <div className="space-y-4 pt-2">
                                            <div className="flex items-center min-h-[24px] overflow-hidden">
                                                {project.slugs && (
                                                    <>
                                                        <img
                                                            src={`https://skillicons.dev/icons?i=${project.slugs}&theme=light`}
                                                            alt={`${project.title} development bundle badges`}
                                                            className="h-7 object-contain opacity-90 group-hover:opacity-100 transition-all duration-300 dark:hidden"
                                                        />
                                                        <img
                                                            src={`https://skillicons.dev/icons?i=${project.slugs}&theme=dark`}
                                                            alt={`${project.title} development bundle badges`}
                                                            className="h-7 object-contain opacity-90 group-hover:opacity-100 transition-all duration-300 hidden dark:inline-block"
                                                        />
                                                    </>
                                                )}
                                            </div>

                                            <div className="flex items-center justify-between border-t border-zinc-200/55 dark:border-zinc-700/55 pt-4">
                                                <div className="flex items-center gap-1.5 font-mono text-[9px] text-zinc-400 dark:text-zinc-500">
                                                    <Calendar className="w-3.5 h-3.5" />
                                                    <span>{project.meta.startTimeline} – {project.meta.endTimeline}</span>
                                                    <span className="text-zinc-300 dark:text-zinc-600">•</span>
                                                    <span className="text-zinc-500 dark:text-zinc-400 font-bold uppercase tracking-wider">{project.meta.role}</span>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-3.5">
                                                    ...
                                                    {project.links.live && (
                                                        <a
                                                            ...
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="flex items-center gap-0.5 text-xs font-bold text-zinc-900 dark:text-zinc-200 hover:text-[#92FE9D] dark:hover:text-[#92FE9D] transition-colors group/link"
                                                        >
                ...
                <div className="flex items-center justify-between border-t border-zinc-200/50 dark:border-zinc-700/50 pt-6 px-2">
                    {/* Dot Indicators */}
                        ...
                        {sortedProjects.map((_, idx) => (
                            <button
                                ...
                                className={`h-2 rounded-full transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                                    activeIndex === idx
                                        ? "w-10 bg-gradient-to-r from-[#00C9FF] to-[#92FE9D] opacity-100"
                                        : "w-2 bg-zinc-300 dark:bg-zinc-600 hover:bg-zinc-400 dark:hover:bg-zinc-500 opacity-60"
                                }`}
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="w-11 h-11 rounded-full border border-zinc-200 dark:border-zinc-700 bg-white/80 dark:bg-zinc-800/80 backdrop-blur-md flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:text-[#00C9FF] hover:border-zinc-300 dark:hover:border-zinc-600 active:scale-95 shadow-[0_4px_12px_rgba(0,0,0,0.02)] transition-all"
